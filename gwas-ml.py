...
def plot_performance(r2, mae, model_name, save_dir="data/image"):
    ...
    # 🔽 Tambahan: cek file yang sudah ada, kasih nomor urut
    base_path = os.path.join(save_dir, f"{model_name}_performance")
    file_path = base_path + ".png"
    counter = 1
    while os.path.exists(file_path):
        file_path = f"{base_path}{counter}.png"
        counter += 1

    plt.savefig(file_path)
    plt.close()
    print(f"Grafik performa disimpan: {file_path}")
